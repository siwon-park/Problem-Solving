...
####################################################
    ...
    # 왜냐하면 불이 퍼진 시간과 지훈이의 시간 차이가 1이면, 지훈이가 일단 먼저 해당 칸으로 이동하고 나서 불이 해당 칸으로 +1 시간 뒤에 퍼져서 지훈이는 불에 타버린다.
    # 예를 들어) 케이스가 아래와 같다고 할 때, (1,1)에 있는 J가 (0,1), (1,0), (1,2), (2,1)로 이동한다고 치면, +1 시간 뒤에 불이 해당 칸으로 번지게 된므로 "IMPOSSIBLE"을 반환해야한다.
    # F.F    0 1 0
    # .J.    1 J 1     => "IMPOSSIBLE"
    # F.F    0 1 0
####################################################
